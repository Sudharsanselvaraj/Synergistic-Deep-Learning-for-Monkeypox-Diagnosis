...
def _load(bb: str):
    F = CFG.features
    return (
        np.load(F / f"X_train_{bb}.npy"),
        np.load(F / "y_train.npy"),
        np.load(F / f"X_val_{bb}.npy"),
        np.load(F / "y_val.npy"),
        np.load(F / f"X_test_{bb}.npy"),
        np.load(F / "y_test.npy"),
    )
...
def train_one(bb: str) -> dict:
    ...
    callbacks = [
        tf.keras.callbacks.EarlyStopping(
            monitor="val_loss", patience=CFG.early_stop_patience, restore_best_weights=True
        ),
        tf.keras.callbacks.ReduceLROnPlateau(
            monitor="val_loss", factor=0.3, patience=4, min_lr=1e-6
        ),
        tf.keras.callbacks.ModelCheckpoint(str(ckpt), monitor="val_accuracy", save_best_only=True),
    ]
    head.fit(
        Xtr,
        ytr,
