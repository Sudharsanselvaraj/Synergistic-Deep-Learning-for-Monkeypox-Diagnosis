...
def _splits(X, y):
    Xtr, Xtmp, ytr, ytmp = train_test_split(X, y, test_size=0.25, random_state=CFG.seed, stratify=y)
    Xva, Xte, yva, yte = train_test_split(
        Xtmp, ytmp, test_size=0.5, random_state=CFG.seed, stratify=ytmp
    )
...
def _fit_cnn(Xtr, ytr, Xva, yva):
    ...
    m.fit(
        ...
        epochs=CFG.symptom_epochs,
        batch_size=CFG.symptom_batch,
...
def main() -> None:
    ...
    print(
        f"[i] {X.shape[1]} features, naive-majority baseline = {100 * max(y.mean(), 1 - y.mean()):.1f}%"
    )
    ...
    rows.append(_score_row("CNN (proposed)", yte, (p > 0.5).astype(int), np.c_[1 - p, p]))
    ...
    for name, clf in [
        ...
        (
            ...
            RandomForestClassifier(
                n_estimators=300, class_weight="balanced", random_state=CFG.seed
            ),
        ),
        (
            "XGBoost",
